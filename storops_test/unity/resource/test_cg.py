...
class UnityConsistencyGroupTest(TestCase):
    ...
    def test_modify_lun_compression_enabled_in_cg(self):
        ...
        resp = lun_in_cg.modify(is_compression_enabled=True)
        assert_that(resp.is_ok(), equal_to(True))

    @patch_rest
    def test_modify_lun_compression_enabled_in_cg_v4_3(self):
        lun_in_cg = UnityLun.get(cli=t_rest(version='4.3'), _id='sv_18')
        resp = lun_in_cg.modify(is_compression_enabled=True)
        assert_that(resp.is_ok(), equal_to(True))

    @patch_rest
    def test_modify_lun_data_reduction_enabled_in_cg(self):
        lun_in_cg = UnityLun.get(cli=t_rest(version='4.3'), _id='sv_18')
        resp = lun_in_cg.modify(is_data_reduction_enabled=True)
        assert_that(resp.is_ok(), equal_to(True))
